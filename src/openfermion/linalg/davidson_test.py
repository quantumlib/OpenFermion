...
def get_difference(linear_operator, eigen_values, eigen_vectors):
    ...
    return numpy.max(numpy.abs(linear_operator.dot(eigen_vectors) - eigen_vectors * eigen_values))
...
class DavidsonOptionsTest(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.eps = 1e-7
        self.davidson_options = DavidsonOptions(self.max_subspace, self.max_iterations, self.eps)

    def test_init(self):
        """Tests vars in __init__()."""
        self.assertEqual(self.davidson_options.max_subspace, self.max_subspace)
        self.assertEqual(self.davidson_options.max_iterations, self.max_iterations)
...
class DavidsonTest(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.davidson = Davidson(
            linear_operator=self.linear_operator, linear_operator_diagonal=self.diagonal
        )
    ...
    def test_init(self):
        ...
        self.assertAlmostEqual(numpy.max(numpy.abs(self.matrix - self.matrix.T)), 0)

        self.assertTrue(davidson.linear_operator)
        self.assertTrue(numpy.allclose(davidson.linear_operator_diagonal, self.diagonal))
    ...
    def test_with_built_in(self):
        ...
        self.assertAlmostEqual(
            get_difference(self.davidson.linear_operator, eigen_values, eigen_vectors), 0
        )
    ...
    def test_lowest_invalid_shape(self):
        ...
        with self.assertRaises(ValueError):
            self.davidson.get_lowest_n(1, numpy.ones((self.matrix.shape[0] * 2, 1), dtype=complex))

    def test_get_lowest_n_trivial_guess(self):
        """Test for get_lowest_n() with trivial initial guess."""
        with self.assertRaises(ValueError):
            self.davidson.get_lowest_n(1, numpy.zeros((self.matrix.shape[0], 1), dtype=complex))
    ...
    def test_get_lowest_two(self):
        ...
        self.assertTrue(
            numpy.allclose(
                self.davidson.linear_operator * eigen_vectors, eigen_vectors * eigen_values
            )
        )
...
class QubitDavidsonTest(unittest.TestCase):
    ...
    def test_get_lowest_n(self):
        ...
        for i in range(min(self.n_qubits, 4)):
            ...
            qubit_operator += QubitOperator(((i, 'Z'),), numpy.random.rand(1)[0])
        ...
        self.assertTrue(numpy.allclose(eigen_values, expected_eigen_values))
        self.assertAlmostEqual(
            get_difference(davidson.linear_operator, eigen_values, eigen_vectors), 0
        )
    ...
    def test_get_lowest_z_real(self):
        ...
        self.assertTrue(numpy.allclose(numpy.real(eigen_vectors), eigen_vectors))
    ...
    def test_get_lowest_y_real_fail(self):
        ...
        self.assertFalse(numpy.allclose(numpy.real(eigen_vectors), eigen_vectors))
...
class SparseDavidsonTest(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.davidson_options = DavidsonOptions(max_subspace=100, max_iterations=50, real_only=True)

        # Checks for built-in eigh() function.
        self.eigen_values, self.eigen_vectors = numpy.linalg.eigh(self.sparse_matrix)
        self.assertAlmostEqual(
            get_difference(self.sparse_matrix, self.eigen_values, self.eigen_vectors), 0
        )
    ...
    def test_hermitain(self):
        ...
        self.assertTrue(numpy.allclose(self.sparse_matrix, self.sparse_matrix.conj().T))

    def test_get_lowest_n_coo(self):
        """Test for get_lowest_n() as a coo_matrix."""
        davidson = SparseDavidson(
            scipy.sparse.coo_matrix(self.sparse_matrix), self.davidson_options
        )

        n_lowest = 2
        initial_guess = numpy.eye(self.dimension, n_lowest)
        success, eigen_values, eigen_vectors = davidson.get_lowest_n(n_lowest, initial_guess)

        expected_eigen_values = self.eigen_values[:n_lowest]

        self.assertTrue(success)
        self.assertLess(
            numpy.max(numpy.abs(eigen_values - expected_eigen_values)), self.davidson_options.eps
        )
        self.assertLess(
...
class DavidsonUtilityTest(unittest.TestCase):
    ...
    def test_append_random_vectors_0(self):
        ...
        self.assertTrue(numpy.allclose(append_random_vectors(vectors, 0), vectors))
    ...
    def test_append_random_vectors(self):
        ...
        self.assertTrue(
            numpy.allclose(
                numpy.dot(new_vectors.conj().T, new_vectors), numpy.eye(col + add, col + add)
            )
        )
    ...
    def test_orthonormalize(self):
        ...
        expected_array = numpy.array(
            [[sqrt_half, sqrt_half, 0], [sqrt_half, -sqrt_half, 0], [0, 0, 1]]
        )

        array = numpy.array([[1, 1, 10, 1], [1, -1, 10, 1], [0, 0, 2, 1]], dtype=float)
        array[:, 0] *= sqrt_half
    ...
    def test_orthonormalize_complex(self):
        ...
        array = numpy.array([[1.0j, 1.0j, 10], [1.0j, -1.0j, 10], [0, 0, 2]], dtype=complex)
        array[:, 0] *= sqrt_half
        array = orthonormalize(array, 1)
        self.assertTrue(numpy.allclose(array, expected_array))
