...
class FermionSpinOperatorsTest(unittest.TestCase):
    ...
    def test_s_plus_operator(self):
        ...
        expected = FermionOperator(((0, 1), (1, 0))) + FermionOperator(((2, 1), (3, 0)))
    ...
    def test_s_squared_operator(self):
        ...
        s_minus = FermionOperator(((1, 1), (0, 0))) + FermionOperator(((3, 1), (2, 0)))
        s_plus = FermionOperator(((0, 1), (1, 0))) + FermionOperator(((2, 1), (3, 0)))
        s_z = (
            FermionOperator(((0, 1), (0, 0)), 0.5)
            - FermionOperator(((1, 1), (1, 0)), 0.5)
            + FermionOperator(((2, 1), (2, 0)), 0.5)
    ...
    def test_relations(self):
        ...
        self.assertEqual(normal_ordered(sx), normal_ordered(0.5 * (s_plus + s_minus)))
        self.assertEqual(normal_ordered(sy), normal_ordered((0.5 / 1.0j) * (s_plus - s_minus)))
        self.assertEqual(normal_ordered(s_squared), normal_ordered(sx**2 + sy**2 + sz**2))
        self.assertEqual(
            normal_ordered(s_squared), normal_ordered(s_plus * s_minus + sz * (sz - identity))
        )
        self.assertEqual(normal_ordered(commutator(s_plus, s_minus)), normal_ordered(2 * sz))
        self.assertEqual(normal_ordered(commutator(sx, sy)), normal_ordered(1.0j * sz))
...
class MajoranaOperatorTest(unittest.TestCase):
    def test_init(self):
        ...
        correct = FermionOperator('3^', 1.0j) - FermionOperator('3', 1.0j)
        self.assertEqual(op1, op2)
        self.assertEqual(op1, correct)
