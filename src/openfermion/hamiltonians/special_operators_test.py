...
from openfermion.hamiltonians.special_operators import (
    majorana_operator, number_operator, s_minus_operator, s_plus_operator,
    s_squared_operator, sx_operator, sy_operator, sz_operator, up_index,
    down_index)
...
class FermionSpinOperatorsTest(unittest.TestCase):
    ...
    def test_s_plus_operator(self):
        ...
        expected = (FermionOperator(((0, 1), (1, 0))) + FermionOperator(
            ((2, 1), (3, 0))))
        self.assertEqual(op, expected)

    def test_s_minus_operator(self):
        op = s_minus_operator(3)
        expected = (FermionOperator(((1, 1), (0, 0))) + FermionOperator(
            ((3, 1), (2, 0))) + FermionOperator(((5, 1), (4, 0))))
        self.assertEqual(op, expected)

    def test_sx_operator(self):
        op = sx_operator(2)
        expected = (FermionOperator(((0, 1), (1, 0)), 0.5) + FermionOperator(
            ((1, 1), (0, 0)), 0.5) + FermionOperator(
                ((2, 1), (3, 0)), 0.5) + FermionOperator(((3, 1), (2, 0)), 0.5))
        self.assertEqual(op, expected)
