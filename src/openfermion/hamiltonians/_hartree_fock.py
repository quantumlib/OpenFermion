...
class HartreeFockFunctional():
    ...
    def __init__(self, *,
        ...
        if initial_orbitals is None:
            ...
            molecular_hamiltonian = generate_hamiltonian(
                one_body_integrals=general_basis_change(self.obi,
                                                        core_orbs,
                                                        (1, 0)),
            ...
            self.hamiltonian = initial_orbitals(self.obi,
                                                self.tbi,
                                                self.num_electrons)
    ...
    def rdms_from_rhf_opdm(self, opdm_aa: np.ndarray) -> InteractionRDM:
        ...
        opdm = np.zeros((2 * self.num_orbitals,
                         2 * self.num_orbitals),
                         dtype=np.complex128)
        opdm[::2, ::2] = opdm_aa
    ...
    def rhf_global_gradient(self, params: np.ndarray,
        ...
        for p in range(self.nocc * self.nvirt):
            ...
            grad_expectation = -1.0 * np.einsum('ab,pa,pb',
                                                self.hamiltonian.one_body_tensor,
                                                pre_matrix_full,
                                                opdm,
                                                optimize='optimal').real

            grad_expectation += 1.0 * np.einsum('ab,bq,aq',
                                                self.hamiltonian.one_body_tensor,
                                                pre_matrix_full,
                                                opdm,
                                                optimize='optimal').real

            grad_expectation += 1.0 * np.einsum('ijkl,pi,jpkl',
                                                self.hamiltonian.two_body_tensor,
                                                pre_matrix_full,
                                                tpdm,
                                                optimize='optimal').real

            grad_expectation += -1.0 * np.einsum('ijkl,pj,ipkl',
                                                 self.hamiltonian.two_body_tensor,
                                                 pre_matrix_full,
                                                 tpdm,
                                                 optimize='optimal').real

            grad_expectation += -1.0 * np.einsum('ijkl,kq,ijlq',
                                                 self.hamiltonian.two_body_tensor,
                                                 pre_matrix_full,
                                                 tpdm,
                                                 optimize='optimal').real

            grad_expectation += 1.0 * np.einsum('ijkl,lq,ijkq',
                                                self.hamiltonian.two_body_tensor,
                                                pre_matrix_full,
                                                tpdm,
                                                optimize='optimal').real
...
def rhf_params_to_matrix(parameters: np.ndarray, num_orbitals: int,
...
def rhf_func_generator(rhf_func: HartreeFockFunctional,
                       ...
                       get_opdm_func: Optional[bool] = False) -> Union[
    Tuple[Callable, Callable, Callable], Tuple[
...
def rhf_minimization(rhf_object: HartreeFockFunctional,
                     ...
                     sp_options: Optional[Union[
                         None, Dict]] = None) -> OptimizeResult:
    """
    ...
    return sp.optimize.minimize(energy, init_guess,
                                jac=gradient,
                                method=method,
