...
from openfermion.hamiltonians.general_hubbard import InteractionParameter, number_operator

from openfermion.ops import FermionOperator


def fermi_hubbard_from_general(
    x_dimension,
    y_dimension,
    tunneling,
    coulomb,
    ...
    lattice = HubbardSquareLattice(x_dimension, y_dimension, periodic=periodic, spinless=spinless)
...
lattices = [
    HubbardSquareLattice(
        random.randrange(3, 10), random.randrange(3, 10), periodic=periodic, spinless=spinless
    )
...
def test_fermi_hubbard_square_lattice_random_parameters(lattice, parameters, distinguish_edges):
    ...
    for term, coefficient in hamiltonian.terms.items():
        ...
        if len(spin_orbitals) == 2:
            (i, a, s), (ii, aa, ss) = (lattice.from_spin_orbital_index(i) for i in spin_orbitals)
            edge_type = (
                {(0, 0): 'onsite', (0, 1): 'vertical_neighbor', (1, 0): 'horizontal_neighbor'}
                if distinguish_edges
                else {(0, 0): 'onsite', (0, 1): 'neighbor', (1, 0): 'neighbor'}
            ...
            if len(term) == 2:
                ...
                spin_pairs = (
                    SpinPairs.ALL
                    if lattice.spinless
                    else (SpinPairs.SAME if s == ss else SpinPairs.DIFF)
                )
            ...
            if not lattice.spinless:
                (-1) ** spin_index
                potential_coefficient -= ((-1) ** spin_index) * parameters.get('magnetic_field', 0)
    ...
    for (term_type, parameter), n_terms in terms_per_parameter.items():
        ...
        if term_type == 'tunneling':
            ...
            if not lattice.spinless:
                ...
                if parameter.edge_type == 'onsite' and parameter.spin_pairs == SpinPairs.DIFF:
                    expected_n_terms *= len(set(parameter.dofs))
                else:
                    expected_n_terms *= 2
