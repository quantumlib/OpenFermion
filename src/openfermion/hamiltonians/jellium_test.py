...
class WignerSeitzRadiusTest(unittest.TestCase):
    def test_wigner_seitz_radius_1d(self):
        ...
        n_particles = 20
        one_d_test = wigner_seitz_length_scale(wigner_seitz_radius, n_particles, 1)
        self.assertAlmostEqual(one_d_test, n_particles * 2.0 * wigner_seitz_radius)

    def test_wigner_seitz_radius_2d(self):
        wigner_seitz_radius = 0.5
        n_particles = 3
        two_d_test = wigner_seitz_length_scale(wigner_seitz_radius, n_particles, 2) ** 2.0
        self.assertAlmostEqual(two_d_test, n_particles * numpy.pi * wigner_seitz_radius**2.0)

    def test_wigner_seitz_radius_3d(self):
        wigner_seitz_radius = 4.6
        n_particles = 37
        three_d_test = wigner_seitz_length_scale(wigner_seitz_radius, n_particles, 3) ** 3.0
        self.assertAlmostEqual(
            three_d_test, n_particles * (4.0 * numpy.pi / 3.0 * wigner_seitz_radius**3.0)
        )
...
class HypercubeGridTest(unittest.TestCase):
    ...
    def test_generation_away_from_half_filling(self):
        ...
        self.assertAlmostEqual(grid.volume_scale(), orbitals * wigner_seitz_radius / 2.5)
    ...
    def test_generation_with_spin(self):
        ...
        self.assertAlmostEqual(grid.volume_scale(), numpy.pi * 16 * 100.0)

    def test_3d_generation_with_rounding(self):
        filling = 0.42
        grid = hypercube_grid_with_given_wigner_seitz_radius_and_filling(
            3, 5, 1.0, filling_fraction=filling
        )
...
class JelliumTest(unittest.TestCase):
    def test_kinetic_integration(self):
        ...
        grid = Grid(dimensions=2, length=2, scale=3.0)
        spinless = False
        ...
        for length in [2, 3, 4]:
            ...
            difference = numpy.amax(numpy.absolute(momentum_spectrum - position_spectrum))
    ...
    def test_potential_integration(self):
        ...
        for length in [2, 3]:
            grid = Grid(dimensions=2, length=length, scale=2.0)
            spinless = True
            momentum_potential = plane_wave_potential(grid, spinless)
            position_potential = dual_basis_potential(grid, spinless)

            # Confirm they are Hermitian
            momentum_potential_operator = get_sparse_operator(momentum_potential)
            self.assertTrue(is_hermitian(momentum_potential_operator))

            position_potential_operator = get_sparse_operator(position_potential)
    ...
    def test_model_integration(self):
        ...
        for length in [2, 3]:
            ...
            # Confirm they are Hermitian
            momentum_hamiltonian_operator = get_sparse_operator(momentum_hamiltonian)
            self.assertTrue(is_hermitian(momentum_hamiltonian_operator))

            position_hamiltonian_operator = get_sparse_operator(position_hamiltonian)
    ...
    def test_model_integration_with_constant(self):
        ...
        for length in [2, 3]:
            ...
            # Hamiltonian.
            momentum_hamiltonian = jellium_model(grid, spinless, True, include_constant=True)
            position_hamiltonian = jellium_model(grid, spinless, False)
    ...
    def test_coefficients(self):
        ...
        paper_kinetic_coefficient = 0.0
        paper_potential_coefficient = 0.0
        for indices in grid.all_points_indices():
            momenta = grid.momentum_vector(indices)
            paper_kinetic_coefficient += (
                float(n_qubits) * momenta.dot(momenta) / float(4.0 * n_orbitals)
            )

            if momenta.any():
                potential_contribution = (
                    -numpy.pi * float(n_qubits) / float(2.0 * momenta.dot(momenta) * volume)
                )
        ...
        for p in range(n_qubits):
            ...
            for indices in grid.all_points_indices():
                momenta = grid.momentum_vector(indices)
                paper_kinetic_coefficient -= momenta.dot(momenta) / float(4.0 * n_orbitals)

                if momenta.any():
                    potential_contribution = numpy.pi / float(momenta.dot(momenta) * volume)
                    paper_potential_coefficient += potential_contribution
                    ...
                    for spin_b in spins:
                        ...
                        for indices_c in grid.all_points_indices():
                            ...
                            if momenta.any():
                                potential_contribution = (
                                    numpy.pi
                                    * numpy.cos(differences.dot(momenta))
                                    / float(momenta.dot(momenta) * volume)
                                )
                                paper_potential_coefficient += potential_contribution
    ...
    def test_jordan_wigner_dual_basis_jellium(self):
        ...
        fermion_hamiltonian = dual_basis_jellium_model(grid, spinless, include_constant=True)
        ...
        if spinless:
            paper_n_terms = 1 - 0.5 * n_qubits + 1.5 * (n_qubits**2)

        num_nonzeros = sum(1 for coeff in qubit_hamiltonian.terms.values() if coeff != 0.0)
    ...
    def test_plane_wave_period_cutoff(self):
        ...
        hamiltonian_2 = jellium_model(grid, spinless, True, False, None, True, period_cutoff)
        jw_2 = jordan_wigner(hamiltonian_2)
        spectrum_2 = eigenspectrum(jw_2)
