...
class RDMMappingTest(unittest.TestCase):
    def setUp(self):
        ...
        tqdm_h2_sto3g = os.path.join(THIS_DIRECTORY, 'testing/tqdm_H2_sto-3g_singlet_1.4.hdf5')
        with h5py.File(tqdm_h2_sto3g, 'r') as fid:
            self.tqdm_h2_sto3g = fid['tqdm'][...]

        phdm_h2_sto3g = os.path.join(THIS_DIRECTORY, 'testing/phdm_H2_sto-3g_singlet_1.4.hdf5')
        with h5py.File(phdm_h2_sto3g, 'r') as fid:
            self.phdm_h2_sto3g = fid['phdm'][...]

        tqdm_h2_6_31g = os.path.join(THIS_DIRECTORY, 'testing/tqdm_H2_6-31g_singlet_0.75.hdf5')
        with h5py.File(tqdm_h2_6_31g, 'r') as fid:
            self.tqdm_h2_6_31g = fid['tqdm'][...]

        phdm_h2_6_31g = os.path.join(THIS_DIRECTORY, 'testing/phdm_H2_6-31g_singlet_0.75.hdf5')
    ...
    def test_tpdm_to_opdm(self):
        ...
        for file in filter(lambda x: x.endswith(".hdf5"), os.listdir(DATA_DIRECTORY)):
            ...
            if molecule.fci_one_rdm is not None and molecule.fci_two_rdm is not None:
                test_opdm = map_two_pdm_to_one_pdm(molecule.fci_two_rdm, molecule.n_electrons)
    ...
    def test_tqdm_conversions_h2_631g(self):
        ...
        test_tqdm = map_two_pdm_to_two_hole_dm(molecule.fci_two_rdm, molecule.fci_one_rdm)
    ...
    def test_phdm_conversions_h2_631g(self):
        ...
        test_phdm = map_two_pdm_to_particle_hole_dm(molecule.fci_two_rdm, molecule.fci_one_rdm)
        assert numpy.allclose(test_phdm, true_phdm)

        test_opdm = map_particle_hole_dm_to_one_pdm(
            true_phdm, molecule.n_electrons, molecule.n_qubits
        )
        assert numpy.allclose(test_opdm, molecule.fci_one_rdm)
