...
def uccsd_convert_amplitude_format(single_amplitudes, double_amplitudes):
    ...
    for i, j, k, l in zip(*double_amplitudes.nonzero()):
        double_amplitudes_list.append([[i, j, k, l], double_amplitudes[i, j, k, l]])
...
def uccsd_singlet_get_packed_amplitudes(
    ...
    for p, q in itertools.product(range(n_virtual), range(n_occupied)):
        ...
        doubles_1.append(double_amplitudes[virtual_up, occupied_up, virtual_down, occupied_down])
    ...
    for (p, q), (r, s) in itertools.combinations(
        ...
        # Get amplitude
        doubles_2.append(
            double_amplitudes[virtual_1_up, occupied_1_up, virtual_2_up, occupied_2_up]
        )

    return singles + doubles_1 + doubles_2


def uccsd_singlet_generator(packed_amplitudes, n_qubits, n_electrons, anti_hermitian=True):
    ...
    for i, (p, q) in enumerate(itertools.product(range(n_virtual), range(n_occupied))):
        ...
        for spin in range(2):
            ...
            if anti_hermitian:
                generator += FermionOperator(((occupied_this, 1), (virtual_this, 0)), -coeff)

            # Generate double excitation
            coeff = t2_1[i]
            generator += FermionOperator(
                ((virtual_this, 1), (occupied_this, 0), (virtual_other, 1), (occupied_other, 0)),
                coeff,
            )
            if anti_hermitian:
                generator += FermionOperator(
    ...
    for i, ((p, q), (r, s)) in enumerate(
        ...
        for spin_a, spin_b in itertools.product(range(2), repeat=2):
            ...
            if occupied_1_a == occupied_2_b:
                ...
                generator += FermionOperator(
                    ((virtual_1_a, 1), (occupied_1_a, 0), (virtual_2_b, 1), (occupied_2_b, 0)),
                    coeff,
                )
                if anti_hermitian:
                    generator += FermionOperator(
                        ((occupied_2_b, 1), (virtual_2_b, 0), (occupied_1_a, 1), (virtual_1_a, 0)),
                        -coeff,
                    )
