...
def fourier_transform_matrix(n_modes):
    ...
    return numpy.array([[root_of_unity**(j * k)
...
@pytest.mark.parametrize(
    'transformation_matrix, initial_state, correct_state', [
        (fourier_transform_matrix(3), 4,
         numpy.array([0, 1, 1, 0, 1, 0, 0, 0]) / numpy.sqrt(3)),
        (fourier_transform_matrix(3), [1, 2],
         numpy.array([
             0, 0, 0,
             numpy.exp(2j * numpy.pi / 3) - 1, 0,
...
@pytest.mark.parametrize('n_spatial_orbitals, conserves_particle_number',
                         [(4, True), (5, True)])
...
@pytest.mark.parametrize('n_qubits, conserves_particle_number', [(4, True),
                                                                 (4, False),
                                                                 (5, True),
                                                                 (5, False)])
