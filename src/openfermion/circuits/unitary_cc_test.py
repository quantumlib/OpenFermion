...
from openfermion.hamiltonians.special_operators import s_squared_operator, sz_operator
...
class UnitaryCC(unittest.TestCase):
    def test_uccsd_anti_hermitian(self):
        ...
        self.assertEqual(generator, -1.0 * conj_generator)
    ...
    def test_uccsd_singlet_anti_hermitian(self):
        ...
        packed_amplitude_size = uccsd_singlet_paramsize(test_orbitals, test_electrons)

        packed_amplitudes = randn(int(packed_amplitude_size))

        generator = uccsd_singlet_generator(packed_amplitudes, test_orbitals, test_electrons)
    ...
    def test_uccsd_singlet_builds(self):
        ...
        generator = uccsd_singlet_generator(initial_amplitudes, n_orbitals, n_electrons)

        test_generator = (
            FermionOperator("2^ 0", 1.0)
            + FermionOperator("0^ 2", -1.0)
            + FermionOperator("3^ 1", 1.0)
            + FermionOperator("1^ 3", -1.0)
            + FermionOperator("2^ 0 3^ 1", 4.0)
            + FermionOperator("1^ 3 0^ 2", -4.0)
        )

        self.assertEqual(normal_ordered(test_generator), normal_ordered(generator))
        ...
        test_generator = (
            ...
            + FermionOperator("3^ 1", 1.0)
            + FermionOperator("1^ 3", -1.0)
            + FermionOperator("4^ 0", 2.0)
            + FermionOperator("0^ 4", -2)
            + FermionOperator("5^ 1", 2.0)
            + FermionOperator("1^ 5", -2.0)
            + FermionOperator("2^ 0 3^ 1", 6.0)
            + FermionOperator("1^ 3 0^ 2", -6.0)
            + FermionOperator("4^ 0 5^ 1", 8.0)
            + FermionOperator("1^ 5 0^ 4", -8.0)
            + FermionOperator("2^ 0 5^ 1", 5.0)
            + FermionOperator("1^ 5 0^ 2", -5.0)
            + FermionOperator("4^ 0 3^ 1", 5.0)
            + FermionOperator("1^ 3 0^ 4", -5.0)
            + FermionOperator("2^ 0 4^ 0", 5.0)
            + FermionOperator("0^ 4 0^ 2", -5.0)
            + FermionOperator("3^ 1 5^ 1", 5.0)
            + FermionOperator("1^ 5 1^ 3", -5.0)
        )
    ...
    def test_sparse_uccsd_generator_numpy_inputs(self):
        ...
        generator = uccsd_generator(sparse_single_amplitudes, sparse_double_amplitudes)

        test_generator = (
            0.12345 * FermionOperator("3^ 5")
    ...
    def test_sparse_uccsd_generator_list_inputs(self):
        ...
        sparse_double_amplitudes = [[[0, 12, 6, 2], 0.3434], [[1, 4, 6, 13], -0.23423]]
    ...
    def test_uccsd_singlet_get_packed_amplitudes(self):
        ...
        packed_amplitudes = uccsd_singlet_get_packed_amplitudes(
            sparse_single_amplitudes, sparse_double_amplitudes, test_orbitals, test_electrons
        )

        self.assertEqual(len(packed_amplitudes), 5)

        self.assertTrue(
            numpy.allclose(packed_amplitudes, numpy.array([0.12345, 0.0, 0.9, 0.0, 0.3434]))
        )
    ...
    def test_ucc_h2(self):
        ...
        expected_uccsd_energy = expectation(self.hamiltonian_matrix, uccsd_state)
        self.assertAlmostEqual(expected_uccsd_energy, self.molecule.fci_energy, places=4)
        ...
        ccsd_operator = uccsd_generator(
            self.molecule.ccsd_single_amps, self.molecule.ccsd_double_amps, anti_hermitian=False
        )

        ccsd_sparse_r = jordan_wigner_sparse(ccsd_operator)
        ccsd_sparse_l = jordan_wigner_sparse(-hermitian_conjugated(ccsd_operator))

        ccsd_state_r = scipy.sparse.linalg.expm_multiply(ccsd_sparse_r, hf_state)
        ccsd_state_l = scipy.sparse.linalg.expm_multiply(ccsd_sparse_l, hf_state)
    ...
    def test_value_error_bad_amplitudes(self):
        with self.assertRaises(ValueError):
            _ = uccsd_singlet_generator([1.0], 3, 4)
