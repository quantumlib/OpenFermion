...
"""Tests for diagonal_coulomb_trotter_error.py."""
...
from openfermion.circuits.trotter.diagonal_coulomb_trotter_error import (
...
class BitMaskModesActedOnByFermionTermsTest(unittest.TestCase):
    ...
    def test_identity_masks_no_modes(self):
        mask = bit_mask_of_modes_acted_on_by_fermionic_terms([FermionOperator.zero()], n_qubits=3)

        self.assertTrue(numpy.array_equal(mask, numpy.zeros((3, 1))))

    def test_mask_single_term(self):
        mask = bit_mask_of_modes_acted_on_by_fermionic_terms([FermionOperator('0^ 0')], n_qubits=2)
        self.assertTrue(numpy.array_equal(mask, numpy.array([[True], [False]])))
    ...
    def test_mask_n_qubits_too_small_for_term(self):
        with self.assertRaises(ValueError):
            bit_mask_of_modes_acted_on_by_fermionic_terms([FermionOperator('1^ 1')], n_qubits=1)

    def test_mask_long_arbitrary_terms(self):
        operator1 = FermionOperator('6^ 5^ 4^ 3 2 1', 2.3 - 1.7j)
        operator2 = FermionOperator('7^ 5^ 1^ 0^', 0.0)

        mask = bit_mask_of_modes_acted_on_by_fermionic_terms([operator1, operator2], n_qubits=8)
    ...
    def test_bit_mask_n_qubits_not_specified(self):
        ...
        self.assertTrue(numpy.array_equal(mask, numpy.array([[True], [False], [True]])))
...
class FermionicSwapNetworkTrotterErrorTest(unittest.TestCase):
    def test_1D_jellium_trotter_error_matches_low_depth_trotter_error(self):
        hamiltonian = normal_ordered(
            jellium_model(
                hypercube_grid_with_given_wigner_seitz_radius_and_filling(
                    1, 5, wigner_seitz_radius=10.0, spinless=True
                ),
                spinless=True,
                plane_wave=False,
            )
        ...
        error_operator = fermionic_swap_trotter_error_operator_diagonal_two_body(hamiltonian)
    ...
    def test_hubbard_trotter_error_matches_low_depth_trotter_error(self):
        hamiltonian = normal_ordered(fermi_hubbard(3, 3, 1.0, 2.3))
...
class SplitOperatorTrotterErrorTest(unittest.TestCase):
    def test_split_operator_error_operator_TV_order_against_definition(self):
        hamiltonian = (
            normal_ordered(fermi_hubbard(3, 3, 1.0, 4.0)) - 2.3 * FermionOperator.identity()
        )
        potential_terms, kinetic_terms = diagonal_coulomb_potential_and_kinetic_terms_as_arrays(
            hamiltonian
        )
        ...
        error_operator = split_operator_trotter_error_operator_diagonal_two_body(
        ...
        error_operator_definition = normal_ordered(commutator(kinetic, inner_commutator))
        error_operator_definition += normal_ordered(commutator(potential, inner_commutator)) / 2.0
    ...
    def test_split_operator_error_operator_VT_order_against_definition(self):
        ...
        error_operator_definition = normal_ordered(commutator(potential, inner_commutator))
        error_operator_definition += normal_ordered(commutator(kinetic, inner_commutator)) / 2.0
        error_operator_definition /= 12.0

        self.assertEqual(error_operator, error_operator_definition)

    def test_intermediate_interaction_hubbard_TV_order_has_larger_error(self):
        hamiltonian = normal_ordered(fermi_hubbard(4, 4, 1.0, 4.0))

        TV_error_operator = split_operator_trotter_error_operator_diagonal_two_body(
            hamiltonian, order='T+V'
        )
        TV_error_bound = numpy.sum(numpy.absolute(list(TV_error_operator.terms.values())))

        VT_error_operator = split_operator_trotter_error_operator_diagonal_two_body(
            hamiltonian, order='V+T'
        )
        VT_error_bound = numpy.sum(numpy.absolute(list(VT_error_operator.terms.values())))
    ...
    def test_strong_interaction_hubbard_VT_order_gives_larger_error(self):
        hamiltonian = normal_ordered(fermi_hubbard(4, 4, 1.0, 10.0))
    ...
    def test_jellium_wigner_seitz_10_VT_order_gives_larger_error(self):
        hamiltonian = normal_ordered(
            jellium_model(
                hypercube_grid_with_given_wigner_seitz_radius_and_filling(
                    2, 3, wigner_seitz_radius=10.0, spinless=True
                ),
                spinless=True,
                plane_wave=False,
            )
