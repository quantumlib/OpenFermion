...
from openfermion.transforms.repconversions.weyl_ordering import (
    mccoy,
    symmetric_ordering,
...
class McCoyTest(unittest.TestCase):
    ...
    def test_mixed(self):
        ...
        n = 1
        res = mccoy(self.mode, self.op_a, self.op_b, m, n)
        expected = {((0, self.op_a), (0, self.op_b)): 0.5, ((0, self.op_b), (0, self.op_a)): 0.5}
        self.assertEqual(res, expected)
...
class SymmetricOrderingTest(unittest.TestCase):
    ...
    def test_symmetric_non_hermitian(self):
        op = BosonOperator('0^ 0')
        res = symmetric_ordering(op)
        expected = BosonOperator('0^ 0', 0.5) + BosonOperator('0 0^', 0.5)
        self.assertTrue(res == expected)
        self.assertTrue(is_hermitian(res))

        op = BosonOperator('0^ 0', 0.5)
        res = symmetric_ordering(op, ignore_coeff=False)
        expected = BosonOperator('0^ 0', 0.25) + BosonOperator('0 0^', 0.25)
        self.assertTrue(res == expected)
        self.assertTrue(is_hermitian(res))
