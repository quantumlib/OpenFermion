...
def bravyi_kitaev_fast_interaction_op(iop: InteractionOperator) -> QubitOperator:
    r"""
    ...
    edge_matrix_indices = numpy.array(
        numpy.nonzero(numpy.triu(edge_matrix) - numpy.diag(numpy.diag(edge_matrix)))
    )
    ...
    for p in range(n_qubits):
        for q in range(n_qubits):
            ...
            if coefficient and p >= q:
                qubit_operator += coefficient * _one_body(edge_matrix_indices, p, q)
...
def bravyi_kitaev_fast_edge_matrix(
    iop: InteractionOperator, n_qubits: Optional[int] = None
) -> numpy.ndarray:
    """
                ...
                for s in range(n_qubits):
                    ...
                    if len(set([p, q, r, s])) == 4:
                        if coefficient2 and p >= q:
                            edge_matrix[p, q] = bool(complex(iop[(p, 1), (q, 1), (r, 0), (s, 0)]))
                            a, b = sorted([r, s])
                            edge_matrix[b, a] = bool(complex(iop[(p, 1), (q, 1), (r, 0), (s, 0)]))
...
def _one_body(edge_matrix_indices: numpy.ndarray, p: int, q: int) -> QubitOperator:
    r"""
    ...
    if p != q:
        ...
        qubit_operator += -1j / 2.0 * (A_ab * B_b + B_a * A_ab)

    # Handle diagonal terms.
    else:
        B_p = edge_operator_b(edge_matrix_indices, p)
        qubit_operator += (QubitOperator((), 1) - B_p) / 2.0

    return qubit_operator


def _two_body(edge_matrix_indices: numpy.ndarray, p: int, q: int, r: int, s: int) -> QubitOperator:
    r"""
    ...
    # Handle case of three unique indices.
        ...
        if p == r:
            ...
            qubit_operator += 1j * (A_qs * B_s + B_q * A_qs) * (QubitOperator(()) - B_p) / 4.0
            ...
            qubit_operator += -1j * (A_qr * B_r + B_q * A_qr) * (QubitOperator(()) - B_p) / 4.0
            ...
            qubit_operator += -1j * (A_ps * B_s + B_p * A_ps) * (QubitOperator(()) - B_q) / 4.0
            ...
            qubit_operator += 1j * (A_pr * B_r + B_p * A_pr) * (QubitOperator(()) - B_q) / 4.0
    ...
    # Handle case of two unique indices.
        ...
        if p == s:
            B_p = edge_operator_b(edge_matrix_indices, p)
            B_q = edge_operator_b(edge_matrix_indices, q)
            qubit_operator += (QubitOperator(()) - B_p) * (QubitOperator(()) - B_q) / 4.0
...
def edge_operator_b(edge_matrix_indices: numpy.ndarray, i: int) -> QubitOperator:
...
def edge_operator_aij(edge_matrix_indices: numpy.ndarray, i: int, j: int) -> QubitOperator:
    ...
    for edge_index in range(numpy.size(qubit_position_i[0, :])):
        if (
            edge_matrix_indices[int(not (qubit_position_i[0, edge_index]))][
                qubit_position_i[1, edge_index]
            ]
            < j
        ):
    ...
    for edge_index in range(numpy.size(qubit_position_j[0, :])):
        if (
            edge_matrix_indices[int(not (qubit_position_j[0, edge_index]))][
                qubit_position_j[1, edge_index]
            ]
            < i
        ):
    ...
    for stab in stabs:
        ...
        for i in range(numpy.size(stab)):
            if i == (numpy.size(stab) - 1):
                A = (1j) * A * edge_operator_aij(edge_matrix_indices, stab[i], stab[0])
            else:
                A = (1j) * A * edge_operator_aij(edge_matrix_indices, stab[i], stab[i + 1])
...
def number_operator(iop, mode_number=None):
    ...
    if mode_number is None:
        for i in range(n_qubit):
            num_operator += (QubitOperator(()) - edge_operator_b(edge_matrix_indices, i)) / 2.0

    else:
        num_operator += (
            QubitOperator(()) - edge_operator_b(edge_matrix_indices, mode_number)
        ) / 2.0

    return num_operator


def generate_fermions(edge_matrix_indices: numpy.ndarray, i: int, j: int) -> QubitOperator:
    ...
    Return:
        A QubitOperator
    """
    gen_fer_operator = (-1j / 2.0) * (
        edge_operator_aij(edge_matrix_indices, i, j) * edge_operator_b(edge_matrix_indices, j)
        - edge_operator_b(edge_matrix_indices, i) * edge_operator_aij(edge_matrix_indices, i, j)
    )
