...
"""Tests for commutator_diagonal_coulomb_operator.py"""
...
from openfermion.transforms.opconversions.commutator_diagonal_coulomb_operator import (
...
class DiagonalHamiltonianCommutatorTest(unittest.TestCase):
    def test_commutator(self):
        operator_a = (
            ...
            + FermionOperator('3^ 1^ 3 1', 0.09)
            + FermionOperator('2^ 0^ 2 0', 0.126j)
            + FermionOperator('4^ 2^ 4 2')
            + FermionOperator('3^ 0^ 3 0')
        )

        operator_b = (
            FermionOperator('3^ 1', 0.7)
            + FermionOperator('1^ 3', -9.0)
        ...
        result = commutator_ordered_diagonal_coulomb_with_two_body_operator(operator_a, operator_b)
    ...
    def test_nonstandard_second_arg(self):
        ...
        operator_b = FermionOperator('4^ 1^ 3 0', 0.1) - FermionOperator('3^ 0^ 1 0', 0.11)

        reference = (
            FermionOperator('1^ 0^ 1 0', -0.11)
        ...
        res = commutator_ordered_diagonal_coulomb_with_two_body_operator(operator_a, operator_b)
    ...
    def test_integration_jellium_hamiltonian_with_negation(self):
        hamiltonian = normal_ordered(jellium_model(Grid(2, 3, 1.0), plane_wave=False))
        ...
        result = commutator_ordered_diagonal_coulomb_with_two_body_operator(part_a, part_b)

        self.assertTrue(result.isclose(reference))

        negative = commutator_ordered_diagonal_coulomb_with_two_body_operator(part_b, part_a)
        result += negative
    ...
    def test_warning_on_bad_input_first_arg(self):
        with warnings.catch_warnings(record=True) as w:
            ...
            self.assertIn('Defaulted to standard commutator evaluation', str(w[-1].message))
    ...
    def test_integration_random_diagonal_coulomb_hamiltonian(self):
        hamiltonian1 = normal_ordered(
            get_fermion_operator(random_diagonal_coulomb_hamiltonian(n_qubits=7))
        )
