...
class CodeTransformTest(unittest.TestCase):
    def test_tranform_function(self):
        ...
        correct_op = QubitOperator(((1, 'Z'), (2, 'X'), (3, 'X'), (4, 'X')), 0.5) + QubitOperator(
            ((2, 'Y'), (3, 'X'), (4, 'X')), 0.5j
        )
        ...
        op2 = QubitOperator('Z0 Z1 Z2 Z3 Z4 X5', 0.5) - QubitOperator('Z0 Z1 Z2 Z3 Z4 Y5', 0.5j)
    ...
    def test_jordan_wigner(self):
        ...
        qubit_hamiltonian = binary_code_transform(hamiltonian, code)
        self.assertAlmostEqual(gs_energy, eigenspectrum(qubit_hamiltonian)[0])
        self.assertDictEqual(qubit_hamiltonian.terms, jordan_wigner(hamiltonian).terms)
