...
class bravyi_kitaev_fastTransformTest(unittest.TestCase):
    ...
    def test_bravyi_kitaev_fast_edgeoperator_Bi(self):
        ...
        self.assertTrue(qterm_b0 == bksf.edge_operator_b(edge_matrix_indices, 0))
        self.assertTrue(qterm_b1 == bksf.edge_operator_b(edge_matrix_indices, 1))
        self.assertTrue(qterm_b2 == bksf.edge_operator_b(edge_matrix_indices, 2))
        self.assertTrue(qterm_b3 == bksf.edge_operator_b(edge_matrix_indices, 3))
    ...
    def test_bravyi_kitaev_fast_edgeoperator_Aij(self):
        ...
        self.assertTrue(qterm_a01 == bksf.edge_operator_aij(edge_matrix_indices, 0, 1))
        self.assertTrue(qterm_a02 == bksf.edge_operator_aij(edge_matrix_indices, 0, 2))
        self.assertTrue(qterm_a03 == bksf.edge_operator_aij(edge_matrix_indices, 0, 3))
        self.assertTrue(qterm_a12 == bksf.edge_operator_aij(edge_matrix_indices, 1, 2))
        self.assertTrue(qterm_a13 == bksf.edge_operator_aij(edge_matrix_indices, 1, 3))
        self.assertTrue(qterm_a23 == bksf.edge_operator_aij(edge_matrix_indices, 2, 3))
    ...
    def test_bravyi_kitaev_fast_jw_number_operator(self):
        ...
        for i in range(numpy.size(jw_eig_spec)):
            if bool(numpy.size(numpy.where(jw_eig_spec[i] == bravyi_kitaev_fast_eig_spec))):
                evensector += 1
    ...
    def test_bravyi_kitaev_fast_jw_hamiltonian(self):
        ...
        n_qubits = count_qubits(self.molecular_hamiltonian)
        bravyi_kitaev_fast_H = bksf.bravyi_kitaev_fast(self.molecular_hamiltonian)
    ...
    def test_bravyi_kitaev_fast_generate_fermions(self):
        ...
        edge_matrix = bksf.bravyi_kitaev_fast_edge_matrix(self.molecular_hamiltonian)
        edge_matrix_indices = numpy.array(
            numpy.nonzero(numpy.triu(edge_matrix) - numpy.diag(numpy.diag(edge_matrix)))
        )
        fermion_generation_operator = bksf.generate_fermions(edge_matrix_indices, 2, 3)
        fermion_generation_sp_matrix = get_sparse_operator(fermion_generation_operator)
        ...
        bksf_vacuum_state_sp_matrix = get_sparse_operator(bksf_vacuum_state_operator)
        ...
        two_fermion_state = numpy.dot(fermion_generation_matrix, bksf_vacuum_state)
        ...
        tot_fermions = numpy.dot(
            two_fermion_state.conjugate().T, numpy.dot(number_operator_matrix, two_fermion_state)
        )
    ...
    def test_bravyi_kitaev_fast_excitation_terms(self):
        ...
        constant = 0
        one_body = numpy.array([[1, 2, 0, 3], [2, 1, 2, 0], [0, 2, 1, 2.5], [3, 0, 2.5, 1]])
        # No Coloumb interaction
        two_body = numpy.zeros((4, 4, 4, 4))
        molecular_hamiltonian = InteractionOperator(constant, one_body, two_body)
